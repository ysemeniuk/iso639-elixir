defmodule ISO639 do
  @moduledoc """
  This libarary exposes functions to convert ISO 639-1 languge code representation to ISO 639-2 and backwards where it's applicable.
  ...
  # @json_639_1 Application.app_dir(:iso639_elixir, "priv/iso-639/data/iso_639-1.json") |> File.read!() |> Poison.decode!s()
  @json_639_2 Application.app_dir(:iso639_elixir, "priv/iso-639/data/iso_639-2.json")
              |> File.read!()
              |> Poison.decode!()

  @doc """
  to_iso639_1/1 expects as input downcased ISO 639-2 or ISO 639-2/B language code and returns ISO 639-1 code if it exists in standard and `nil` if not. If function receives as input ISO 639-1 language code, it's returned unchanged.
  ## Examples
  ...
  @doc """
  to_iso639_2/1 expects as input downcased ISO 639-1, ISO 639-2 or ISO 639-2/B language code and returns ISO 639-2 code if it exists in standard and `nil` if not. If function receives as input ISO 639-2 language code, it's returned unchanged. ISO 639-2/B will be changed to ISO 639-2
  ## Examples
